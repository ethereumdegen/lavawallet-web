const _0xBitcoinToken = artifacts.require("./_0xBitcoinToken.sol");
const ethUtil = require("ethereumjs-util");
const web3utils = require("web3-utils");
const solidityHelper = require("./solidity-helper");
const miningHelper = require("./mining-helper");
const networkInterfaceHelper = require("./network-interface-helper");
...
contract("_0xBitcoinToken", function(accounts) {
    it("can deploy ", async function() {
            ...
            let tokenContract = await _0xBitcoinToken.deployed();
        }),
        it("can be minted", async function() {
            ...
            let challenge_number = await tokenContract.getChallengeNumber.call();

            //  challenge_number = '0x513d3339b587b62e4ea2b9d2762113a245f9fdad264d37bcc6829ce66bd4d456';
            challenge_number =
                "0x085078f6e3066836445e800334b4186d99567065512edfe78fa7a4f611d51c3d";

            let solution_number = 1185888746;
            let solution_digest =
                "0x000016d56489592359ce8e8b61ec335aeb7b7dd5695da22e25ab2039e02c8976";
            let from_address = "0x2B63dB710e35b0C4261b1Aa4fAe441276bfeb971";
            let targetString = await tokenContract.getMiningTarget.call({
                from: from_address
            });
            let target = web3utils.toBN(targetString);
            ...
            const phraseDigest = web3utils.soliditySha3(
                challenge_number,
                from_address,
                solution_number
            );
            ...
            let checkDigest = await tokenContract.getMintDigest.call(
                solution_number,
                phraseDigest,
                challenge_number, {
                    from: from_address
            ...
            /* var checkSuccess = await tokenContract.checkMintSolution.call(
              solution_number,
              phraseDigest,
              challenge_number,
              target
            ...
            var mint_tokens = await tokenContract.mint.call(
              solution_number,
              phraseDigest,
              { from: from_address }
            );
...
async function printBalances(accounts) {
    ...
    console.log(
        "acct 0 balance",
        web3utils.fromWei(balance_val.toString(), "ether")
    );
    // })
}