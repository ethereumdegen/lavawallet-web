...
let busySendingSolution = false;
let queuedMiningSolutions = [];
let lastSubmittedMiningSolutionChallengeNumber;
...
module.exports = {
        ...
        if (busySendingSolution == false) {
            if (queuedMiningSolutions.length > 0) {
                ...
                if (nextSolution.challenge_number != lastSubmittedMiningSolutionChallengeNumber) {
                    ...
                    try {
                        let response = await this.submitMiningSolution(nextSolution.addressFrom,
    ...
    async submitMiningSolution(addressFrom, solution_number, challenge_digest) {
        ...
        let addressTo = this.tokenContract.address;

        let txData = this.web3.eth.abi.encodeFunctionCall({
            name: 'mint',
            type: 'function',
            inputs: [{
                type: 'uint256',
    ...
    async sendSignedRawTransaction(web3, txOptions, addressFrom, account, callback) {
        ...
        let fullPrivKey = account.privateKey;

        let privKey = this.truncate0xFromString(fullPrivKey);
        ...
        let result = web3.eth.sendSignedTransaction('0x' + serializedTx, callback);
