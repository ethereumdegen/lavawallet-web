...
const solidityHelper = require("./solidity-helper");
...
module.exports = {
    async init(web3, contract, mining_account, networkInterface) {
        ...
        let eth_account = mining_account;

        setInterval(
            function() {
                this.printMiningStats();
            }.bind(this),
            5000
        );
        ...
        let minerEthAddress = eth_account.address;
        ...
        setInterval(function() {
            self.collectDataFromContract(contractData);
        }, 10000);
    ...
    async collectDataFromContract(contractData) {
        ...
        let lastRewardAmountString = await tokenContract.lastRewardAmount.call();
        let lastRewardAmount = parseInt(lastRewardAmountString);

        let miningDifficultyString = await tokenContract.getMiningDifficulty.call();
        let miningDifficulty = parseInt(miningDifficultyString);

        let miningTargetString = await tokenContract.getMiningTarget.call();
        let miningTarget = web3utils.toBN(miningTargetString);

        let challengeNumber = await tokenContract.getChallengeNumber.call();
    ...
    async submitNewMinedBlock(
        ...
        this.networkInterface.queueMiningSolution(
            addressFrom,
            solution_number,
            digest_bytes,
            challenge_number
    ...
    */
    ...
    mineCoins(web3, contractData, minerEthAddress) {
        ...
        let solution_number = web3utils.randomHex(32); //solution_number like bitcoin

        let challenge_number = contractData.challengeNumber;
        let target = contractData.miningTarget;

        let digest = web3utils.soliditySha3(
            challenge_number,
            minerEthAddress,
            solution_number
        );
        ...
        let digestBytes32 = web3utils.hexToBytes(digest);
        ...
        let miningTarget = web3utils.toBN(target).mul(new BN(1));
        ...
        if (digestBigNumber.lt(miningTarget)) {
            ...
            if (this.testMode) {
                this.mining = false;
                this.networkInterface.checkMiningSolution(
                    minerEthAddress,
                    solution_number,
                    web3utils.bytesToHex(digestBytes32),
                    challenge_number,
                    miningTarget,
                    function(result) {
